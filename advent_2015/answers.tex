...
\aproblem{19}{147}{\advent@xv@xix}{
  ...
  Define a recursive function $N(\braces{n_i}, m)$ where $\braces{n_i}$ is the sequence of positive integer denominators of unit fractions so far, and $m$ is the number of fractions remaining.
  Now suppose we have chosen $k$ fractions so far and $m$ remain (so that in our case $k + m = 5$).
  Then the remaining fractions must add to
  \gath{
    1 - \sum_{i=1}^k \frac{1}{n_i}
  }

  Implementing and executing this algorithm in Python gives the answer $147$.
}
