...
\aproblem{19}{784}{\advent@xxv@xix}{
  First, we know that the largest number (and our answer) must be a three-digit number so that we need not consider any square numbers larger than $1000$.
  The set of all square numbers less than $1000$ such that no digits are repeated in a given number are
  \gath{
    \braces{1, 4, 9, 16, 25, 36, 49, 64, 81, 169, 196, 256, 289, 324, 361, 529, 576, 625, 729, 784, 841, 961}. \label{eqn:19:squares}
  }
  Since we must select $5$ of these, we ask how many of one-digit, two-digit, and three-digit numbers we might need to select such that the total number of digits is $9$.
  It turns out there are only two ways to do this:
  \gath{
    \squares{3, 2, 2, 1, 1} \non
    \squares{3, 3, 1, 1, 1}
  }
  where each sequence represents five numbers where the number of digits in each number are the elements of the sequence.
  First consider the second of these, that is $2$ three-digit numbers and $3$ one-digit numbers.
  Looking at \eqref{eqn:19:squares}, there are only $3$ one-digits squares, so that $\braces{1, 4, 9}$ must be $3$ of our numbers.
  The three-digit squares that do not contain any of these digits reduce to
  \gath{
    \braces{256, 576, 625},
  }
  but clearly no two of these can be selected without them sharing a $6$ so that this is not possible.

  Hence, it must be that Eve wrote $1$ three-digit number, $2$ two-digit numbers, and $2$ one-digit numbers.
  For each one or two-digit square, $n$ let $T_n$ be the set of three-digit squares that contain none of the digits of $n$.
  We then have that
  \ali{
    T_{1} &= \braces{256, 289, 324, 529, 576, 625, 729, 784} \non
    T_{4} &= \braces{169, 196, 256, 289, 361, 529, 576, 625, 729, 961} \non
    T_{9} &= \braces{256, 324, 361, 576, 625, 784, 841} \non
    T_{16} &= \braces{289, 324, 529, 729, 784} \non
    T_{25} &= \braces{169, 196, 361, 784, 841, 961} \non
    T_{36} &= \braces{289, 529, 729, 784, 841} \non
    T_{49} &= \braces{256, 361, 576, 625} \non
    T_{64} &= \braces{289, 529, 729} \non
    T_{81} &= \braces{256, 324, 529, 576, 625, 729} \non
  }
  Now, for the single-digit squares $\braces{1, 4, 9}$ we choose two.
  And for that choice, we choose two two-digit squares from $\braces{16, 25, 36, 49, 64, 81}$ that containing neither of the single digits.
  Then we calculate the intersections of the $T_n$ sets for these four squares, to see what, if any, three-digits numbers are viable as the final choice.
  These are summarized below:
  \begin{center}
    \begin{tabular}{cl}
      Choice                  & Intersection                                              \\
      \hline
      $\braces{1, 4, 25, 36}$ & $T_{1} \cap T_{4} \cap T_{25} \cap T_{36} = \es$          \\
      $\braces{1, 9, 25, 64}$ & $T_{1} \cap T_{9} \cap T_{25} \cap T_{64} = \es$          \\
      $\braces{1, 9, 36, 64}$ & $T_{1} \cap T_{9} \cap T_{36} \cap T_{64} = \es$          \\
      $\braces{1, 9, 25, 36}$ & $T_{1} \cap T_{9} \cap T_{25} \cap T_{36} = \braces{784}$ \\
      $\braces{4, 9, 16, 25}$ & $T_{4} \cap T_{9} \cap T_{16} \cap T_{25} = \es$          \\
      $\braces{4, 9, 16, 36}$ & $T_{4} \cap T_{9} \cap T_{16} \cap T_{36} = \es$          \\
      $\braces{4, 9, 16, 81}$ & $T_{4} \cap T_{9} \cap T_{16} \cap T_{81} = \es$          \\
      $\braces{4, 9, 25, 36}$ & $T_{4} \cap T_{9} \cap T_{25} \cap T_{36} = \es$          \\
      $\braces{4, 9, 25, 81}$ & $T_{4} \cap T_{9} \cap T_{25} \cap T_{81} = \es$          \\
      $\braces{4, 9, 36, 81}$ & $T_{4} \cap T_{9} \cap T_{36} \cap T_{81} = \es$          \\
    \end{tabular}
  \end{center}
  Clearly then, the only possible solution is that Eve wrote down the numbers $\braces{1, 9, 25, 36, 784}$ so that our answer is $784$.
  \verified{}
}

\aproblem{20}{196}{\advent@xxv@xx}{
  If our answer is $n$, it is reasonable to assume that the perfect powers separated by $p = 29$ might be consecutive squares since these grow the slowest and so may be most likely to be so close together.
  If we try this, we
  \gath{
    (n+1)^2 - n^2 = p \non
    n^2 + 2n + 1 - n^2 = p \non
  ...
  So we have $n = (29 - 1)/2 = 14$ so that $n^2 = 196$  and $(n+1)^2 = 15^2 = 225$ are perfect powers separated by $29$.
  Hence, $196$ must be our answer, and we are given that this must be unique.
  However, it is unclear how to prove that this is unique or why they must be consecutive squares.
  \verified{}

  It is also worth noting that this same approach yields $n = (23 - 1)/2  = 11$ for the example so that $11^2 = 121$ and $12^2 = 144$ are also perfect powers separated by $23$.
}

\aproblem{21}{638}{\advent@xxv@xxi}{
  This is similar to the Dec~14 problem above, but is more complicated.
  Define a valid sequence of As and Bs as one that does not contain an even number of consecutive As, again not counting zero As as even of course.
  For a sequence of length $n$, let $B(n)$ denote the number of valid sequences that end in a $B$, $A_o(n)$ denote the number of valid sequences ending in an \emph{odd} number of As, and $A_e(n)$ denote the number of otherwise valid sequences that end in an \emph{even} number of As.
  Lastly, let $T(n)$ denote the total number of valid sequences of length $n$.
  First, we clearly have that
  \gath{
    T(n) = B(n) + A_o(n)
  }
  to account for valid sequences that end in a B or an A.
  Now, if we have a sequence of length $n-1$ and tack a B on the end, then it is still valid, and hence
  \gath{
    B(n) = T(n-1).
  }
  We also have
  ...
  since adding an A to a sequence ending in a B makes it a valid sequence ending in an odd number (i.e. one) of As.
  Similarly, an otherwise valid sequence ending in an even number of As becomes fully valid if we add a single A onto the end, and will then end in an odd number of As.
  Lastly, we have
  \gath{
    A_e(n) = A_o(n-1)
  }
  since adding an A to the end of a valid sequence ending in an odd number of As, makes it a sequence that is otherwise valid but ends in an even number of As.
  For a sequence of length $n = 1$ it can either simply be an A or a B, which are both valid, so that
  \ali{
    B(1) &= 1 \non
    A_o(1) &= 1 \non
    A_e(1) &=  0 \non
  ...
  The complex recursive relationships above involving $B(n)$, $A_o(n)$, $A_e(n)$, and $T(n)$ can be reduced down as follows:
  \gath{
    T(n) = B(n) + A_o(n) = T(n-1) + A_o(n)
  }
  and
  \gath{
    A_o(n) = B(n-1) + A_e(n-1) = T(n-2) + A_o(n-2).
  }
  We also of course have
  \ali{
    A_o(2) = B(1) + A_e(1) = 1 + 0 = 1.
  }
  Summarizing and making this a little more abstract by renaming $A_o(n)$ to $S(n)$ for brevity, we get the double recursion
  \ali{
    T(n) &= S(n) + T(n-1) \non
    S(n) &= S(n-2) + T(n-2)
  }
  ...
  Evaluating $T(n)$ using these gives
  \ali{
    T(1) &= 2 & T(2) &= 3 & T(3) &= 6 & T(4) &= 10 \non
    T(5) &= 19 & T(6) &= 33 & T(7) &= 61 & T(8) &= 108 \non
    T(9) &= 197 & T(10) &= 352 & T(11) &= 638
  }
  so that our answer, the number of valid sequences of length $11$, is $T(11) = 638$, noting also that $T(4) = 10$ as expected for the example.
  \verified{}
}

\aproblem{22}{504}{\advent@xxv@xxii}{
  \boxans{\gridsol@advent@xxv@xxii}
}

\aproblem{23}{407}{\advent@xxv@xxiii}{
  As no analytical solution is readily apparent, this was solved using a brute force approach in Python.
}

\aproblem{24}{675}{\advent@xxv@xxiv}{
  Since we are only interested in numbers that have both $3$ and $5$ as prime factors and no other prime factors, we need only consider numbers of the form $3^k 5^n$.
  Moreover, $3^6 = 729$ but $3^7 = 2187$, so, if $k$ is the power of $3$, we need only consider $k \in I_6 = \braces{1, 2, \ldots, 6}$.
  Similarly, $5^4 = 625$ but $5^5 = 3125$ so, for $n$ the power of $5$, we need only consider $n \in I_4 = \braces{1, 2, 3, 4}$.
  If we list all $3^k 5^n$ where $(k, n) \in I_6 \times I_4$, we get
  \ali{
    3^{1} 5^{1} &= 15 & 3^{1} 5^{2} &= 75 & 3^{1} 5^{3} &= 375 & 3^{1} 5^{4} &= 1875 \non
    3^{2} 5^{1} &= 45 & 3^{2} 5^{2} &= 225 & 3^{2} 5^{3} &= 1125 & 3^{2} 5^{4} &= 5625 \non
    3^{3} 5^{1} &= 135 & 3^{3} 5^{2} &= 675 & 3^{3} 5^{3} &= 3375 & 3^{3} 5^{4} &= 16875 \non
    3^{4} 5^{1} &= 405 & 3^{4} 5^{2} &= 2025 & 3^{4} 5^{3} &= 10125 & 3^{4} 5^{4} &= 50625 \non
    3^{5} 5^{1} &= 1215 & 3^{5} 5^{2} &= 6075 & 3^{5} 5^{3} &= 30375 & 3^{5} 5^{4} &= 151875 \non
    3^{6} 5^{1} &= 3645 & 3^{6} 5^{2} &= 18225 & 3^{6} 5^{3} &= 91125 & 3^{6} 5^{4} &= 455625.
  }
  Clearly the largest three-digit number here is our answer $3^3 5^2 = 675$.
  \verified{}
}
