...
parser = argparse.ArgumentParser(description="Generates LaTeX code for math puzzle questions with particular tags, or the complete set, in order. Tag data is read from '" + fname + "'.")
parser.add_argument("tags", metavar="TAGS", type=str, nargs="?", help="CSV list of tags to include")
parser.add_argument("--info", "-i", action="store_true", help="Show info, including available tags, question sets, and completeness of those sets.")
...
if args.info :
    ...
    for sett in sets :
        ...
        print(set_type_labels[sett.type], str(sett.year) + ":", len(nums), "puzzle" + ("s" if len(nums) > 1 else ""), "(" + ("Complete" if len(vs) == 0 else ", ".join(vs)) + ")")
else :
    # Generate LaTeX code
    for year in sorted(set([s.year for s in sets])) :
        for typ in set_type_labels.keys() :
            ...
            if len(ss) > 1 :
                raise ValueError("There are multiple " + set_type_labels[typ] + " " + str(year) + " sets!")
            for s in ss :
                for p in sorted(s.puzzles, key=lambda p : p.number) :
                    if len(p.tags.intersection(include_tags)) < 1 :
                        continue
                    label = set_type_labels[s.type] + " " + str(s.year) + ", " + puzzle_labels[s.type] + " " + str(p.number)
                    quest = "@".join([s.type.value, roman.toRoman(s.year - 2000).lower(), roman.toRoman(p.number).lower()])
                    tags = ", ".join(sorted([tag_labels[t] for t in p.tags]))
                    print(r"\puzzle{" + label + "}{\\" + quest + "}{" + tags + "}")
