...
\title{Matthew Scroggs Advent Calendar 2022 Answers}
\author{Dan Whitman}
\date{}
...
\aproblem{6}{128}{\advent@xxii@vi}{
  ...
  and
  \ali{
    N_3(s) &= \sum_{j=2}^{s-1} N_2(j) = \sum_{j=2}^{s-1} (j - 1) = \sum_{j=2}^{s-1} j - \sum_{j=2}^{s-1} 1 = \squares{\frac{s(s-1)}{2} - 1} - (s - 2) \non
...
\aproblem{24}{128}{\advent@xxii@xxiv}{
  From the 2019, Dec 7 problem, the sum of the coefficients of $(ax + b)^n$ is
  \gath{
    S(a, b, n) = \sum_{k=0}^n \binom{n}{k} a^{n - k} b^k.
  }
  So the example becomes
  \gath{
    S(3, 1, 2) = \sum_{k=0}^2 \binom{2}{k} 3^{2 - k} (-1)^k = 9 - 2 \cdot 3 + 1 = 4
  }
  as expected.
  In our case, our answer is
  \gath{
    S(3, 1, 7) = \sum_{k=0}^7 \binom{7}{k} 3^{7 - k} (-1)^k = 128,
  }
