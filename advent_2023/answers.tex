...
\aproblem{20}{240}{\advent@xxiii@xx}{
  ...
  \gath{
    N(1, n) = 1 \non
    N(k, n) = \sum_{m=1}^{n-k+1} {}_mC_n \cdot N(k-1, n-m) = \sum_{m=1}^{n-k+1} \binom{n}{m} N(k-1, n-m). \label{eqn:20:rec}
  }
...
\aproblem{21}{231}{\advent@xxiii@xxi}{
  Let $N(n, k)$ be the number of $n$-digit numbers whose digits are all $1$ through $k$ inclusive, and whose second digit onwards are less than or equal to the previous digit.
  ...
  The first digit $d$ can be $1$ through $k$ inclusive, and, for each the there are then $N(n-1, d)$ $(n-1)$-digit numbers to fill out the remaining digits since the remaining digits can be no larger than $d$.
