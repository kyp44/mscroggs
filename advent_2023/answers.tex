...
\aproblem{22}{TODO}{\advent@xxiii@xxii}{
  Right triangles can only be formed within rectangles, a single rectangle forming $4$ right triangles as in the example.
  So the question can be rephrased as how many rectangles can be formed from the vertices of a regular $n$-gon?
  I suspect that $n$-gons with odd $n$ can form no rectangles on account of having no two sides that are parallel.
  For an even $n$-gon, there are clearly $n/2$ distinct pairs of parallel sides between which the vertices form a distinct rectangle.
  So, if $N(n)$ denotes the number of right triangles that can be formed from the vertices of an even $n$-gon, then it follows that
  \gath{
    N(n) = 4 \cdot \frac{n}{2} = 2n,
  }
  noting that this is only valid when $n > 4$ since, as in the example, $N(4) = 4 \neq 2 \cdot 4 = 8$.
  Thus, regarding the actual question, we have our answer:
  \gath{
    N(n) = 2n = 14620 \non
    n = 14620/2 = 7310.
  }
  TODO!!! Not correct, see octagon, which has more rectangles than just parallel sides!
}
