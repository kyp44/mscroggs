...
We know that if $n$ has a prime factorization of
\gath{
...
Now, the only way to get an odd product is if both of the multiplicands are odd.
From this it follows that every $e_i + 1$ must be odd in order to get an odd number of factors.
Hence, every $e_i$ must be even.
So, supposing that every $e_i = 2k_i$, then we have that
\gath{
n = \prod_{i=1}^N p_i^{e_i} = \prod_{i=1}^N p_i^{2 k_i} = \parens{\prod_{i=1}^N p_i^{k_i}}^2 = k^2,
}
where we set $k = \prod_{i=1}^N p_i^{k_i}$.
This is to say that $n$ must be a square number.

Thus, the question can be rephrased as how many square numbers are there between $1$ and $m = 4008004$.
The square root of $m$ is exactly $2002$, hence our answer is that there are $2002$ square numbers between $1$ and $m$, namely the numbers $i^2$ where $1 \leq i \leq 2002$.
}
