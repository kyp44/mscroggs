...
\cproblem{4}{130405}{\card@xix@iv}{
  In order for a book to even have a center spread it must have at least $4$ pages, where we take a page to be a single side of a two-sided sheet in the book.
  Moreover if the number of pages $N$ is divisible by $4$, then every side of every sheet will be used.
  In our case $N = 130404$, which is divisible by $4$, so we shall only consider this case.
  For a book where $N = 4$ the center pages are $2 = N/2$ and $3 = N/2+1$ so that the sum is
  $$
  s = \frac{N}{2} + \frac{N}{2} + 1 = N + 1 \,.
  $$
  In fact, this generalizes to any book where $N$ is divisible by $4$.
  So in our case our answer is $s = N+1 = 130405$.
}
...
\cproblem{8}{59}{\card@xix@viii}{
  This was calculated in Python, which was made easy by the fact that integers can be arbitrarily large in Python.
}
