...
\title{Matthew Scroggs Advent Calendar 2019 Answers}
\author{Dan Whitman}
\date{}
...
\aproblem{7}{243}{\advent@xix@vii}{
  ...
  \gath{
    (x+y)^n = \sum_{k=0}^n \binom{n}{k} x^{n-k} y^k \,, \label{eqn:pvii:gen}
  }
  where of course $\binom{n}{k}$ are the binomial coefficients.
  Consider now the special case of $x \to ax$ and $y \to b$.
  Then \eqref{eqn:pvii:gen} becomes
  \gath{
    (ax + b)^n = \sum_{k=0}^n \binom{n}{k} (ax)^{n - k} b^k = \sum_{k=0}^n \binom{n}{k} a^{n - k} b^k x^{n - k}
  }
  so that the sum of coefficients is clearly
  \gath{
    S(a, b, n) = \sum_{k=0}^n \binom{n}{k} a^{n - k} b^k.
  }
  Hence, for the example, we have
  \gath{
    S(1, 1, 5) = \sum_{k=0}^5 \binom{5}{k} = 32
  }
  as expected, which was verified using Python.
  For the main problem we then have
  \gath{
    S(2, 1, 5) = \sum_{k=0}^5 \binom{5}{k} 2^{5-k} = \sum_{k=0}^5 \binom{5}{k} 2^k = 243
  }
